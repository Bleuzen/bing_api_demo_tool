...
public class OldMain {
	...
	private static Logger logger = (Logger) LoggerFactory.getLogger(OldMain.class);
	...
	private static HashMap<String, SearchResult> map = new HashMap<>();
	...
	static boolean download(int stopPage, String key, String url) {
        ...
        do {
        	try {
        		...
        		logger.debug("Searching on page {}: {}", currentPage, pageUrl);
                ...
                if(clientIdHeader != null) {
                	...
                	logger.debug("added clientIdHeader");
                }
                if(userStateHeader != null) {
                	request.addHeader(userStateHeader);
                	logger.debug("added userStateHeader");
                }
                ...
                try {
					...
					logger.info("error in request", e);
				}
    			...
    			if(responseCode != 200) {
    				logger.error("Da stimmt was nicht mit dem Statuscode der Antwort von Bing.");
    				return false;
    			}
    			...
    			logger.debug("Bing response: {}", out);
    			...
    			if(totalEstimatedMatches == -1) {
    				...
    				logger.debug("Found totalEstimatedMatches: {}", + totalEstimatedMatches);
    			}
    		} catch (Exception e) {
        	...
        	try {
				...
				logger.debug("Current results: {}", cr);
    		} catch (UnsupportedEncodingException e) {
    			e.printStackTrace();
    		}
        } while((totalEstimatedMatches > map.size()) && (currentPage < stopPage));
        
        logger.info("Search stopped at page: {}", currentPage);
        logger.info("Results: " + map.size());
    ...
    static void duplikateMitAnderemProtokollAusgeben() {
		...
		logger.info("Duplicates with different protocol:");
